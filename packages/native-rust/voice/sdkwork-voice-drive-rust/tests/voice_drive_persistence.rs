use sdkwork_drive_product::application::space_service::DriveSpaceService;
use sdkwork_drive_product::application::workspace_service::DriveWorkspaceService;
use sdkwork_drive_product::infrastructure::sql::space_store::SqlSpaceStore;
use sdkwork_drive_product::infrastructure::sql::uploader_store::SqlUploaderStore;
use sdkwork_drive_product::infrastructure::sql::workspace_store::SqlDriveWorkspaceStore;
use sdkwork_drive_product::uploader::{DriveUploaderService, UploaderActor, UploaderTarget};
...
async fn sql_executor_creates_ai_generated_space_and_drive_upload_item() {
    ...
    let executor = DriveProductVoiceUploadExecutor::new(
...
async fn bytes_persister_writes_multiple_generated_images_to_drive_storage_and_workspace() {
    ...
    let persister = DriveProductVoiceBytesPersister::new(
        DriveSpaceService::new(SqlSpaceStore::new(pool.clone())),
...
impl VoiceDriveUploadExecutor for InMemoryVoiceDriveUploadExecutor {
    ...
    async fn prepare_upload(
        &self,
        command: sdkwork_drive_product::uploader::PrepareUploaderUploadCommand,
    ) -> Result<sdkwork_drive_product::uploader::DriveUploadItem, DriveProductError> {
        self.prepared_uploads.borrow_mut().push(command.id.clone());
        Ok(sdkwork_drive_product::uploader::DriveUploadItem {
            id: command.id,
