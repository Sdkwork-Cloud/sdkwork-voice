...
use sdkwork_drive_product::ports::space_store::DriveSpaceStore;
use sdkwork_drive_product::ports::workspace_store::DriveWorkspaceStore;
...
impl From<DriveProductError> for VoiceDrivePersistenceError {
    fn from(value: DriveProductError) -> Self {
        Self::Drive(value)
    }
}
...
pub struct DriveProductVoiceUploadExecutor<U, P>
where
...
pub struct DriveProductVoiceBytesPersister<P, W, O>
where
    P: DriveSpaceStore,
...
impl<P, W, O> DriveProductVoiceBytesPersister<P, W, O>
where
    P: DriveSpaceStore,
    W: DriveWorkspaceStore,
    O: DriveObjectStore,
...
impl<U, P> DriveProductVoiceUploadExecutor<U, P>
where
    U: DriveUploaderStore,
    P: DriveSpaceStore,
{
...
pub async fn persist_voice_generated_artifact_bytes_to_drive<P, W, O>(
    batch: VoiceGeneratedArtifactBytesBatch,
    persister: &DriveProductVoiceBytesPersister<P, W, O>,
) -> Result<VoiceDriveStoredArtifactBatch, VoiceDrivePersistenceError>
where
    P: DriveSpaceStore,
    W: DriveWorkspaceStore,
    O: DriveObjectStore,
{
    ...
    for (upload, artifact_bytes) in plan.uploads.into_iter().zip(batch.artifacts.into_iter()) {
        ...
        persister
            ...
            .await
            .map_err(|error| {
                VoiceDrivePersistenceError::Drive(DriveProductError::Internal(format!(
    ...
    P: DriveSpaceStore,
{
        ...
        .await
    {
        ...
        Err(DriveProductError::Conflict(_)) => spaces
            .list_spaces(ListSpacesCommand {
                tenant_id: tenant_id.to_string(),
                owner_subject_type: Some(owner_subject_type.to_string()),
                owner_subject_id: Some(owner_subject_id.to_string()),
...
#[async_trait(?Send)]
impl<U, P> VoiceDriveUploadExecutor for DriveProductVoiceUploadExecutor<U, P>
where
    U: DriveUploaderStore,
    P: DriveSpaceStore,
{
    async fn ensure_ai_generated_space(
        ...
        {
            Ok(space) => Ok(space.id),
            Err(DriveProductError::Conflict(_)) => self
                .spaces
                .list_spaces(ListSpacesCommand {
                    tenant_id: tenant_id.to_string(),
                    owner_subject_type: Some(owner_subject_type.to_string()),
